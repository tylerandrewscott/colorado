...
temp = read.csv('input/integrity_db/OID.OperationSearchResults.2016.7.1.3-57 PM.csv',stringsAsFactors = F)

cert_df = temp %>% filter(grepl('United States',Country)|Country=='') %>% select(Certifier.Name) %>% filter(!duplicated(Certifier.Name)) %>%
...
cert_with_no_address = unique(temp$Certifier.Name[temp$Country==''])


test = temp %>% filter(grepl('United States',Country)|Country=='') %>% 
  filter(Certifier.Name %in% cert_with_no_address) %>%
   mutate(Has_Location = Country!='')

table(test$Certifier.Name,test$Has_Location)



temp = temp %>% filter(grepl('United States',Country)|Country=='')




test = temp %>% filter(Country==''|Certifier.Name=='Washington State Department of Agriculture')
names(test)

test$Operation.Name
unique(test$City)
sort(unique(test$Certifier.Name))
